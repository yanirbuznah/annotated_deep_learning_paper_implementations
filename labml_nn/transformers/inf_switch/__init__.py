...
class InfSwitchFeedForward(Module):
    """
    ...
    def forward(self, x: torch.Tensor, dropped: list):
        """
        * `x` is the input to the switching module with shape `[seq_len, batch_size, d_model]`
        ...
        expert_output = [self.experts[i](x[indexes_list[i], :]) for i in range(self.n_experts - 1)]

        dropped += indexes_list[-1].tolist()
        ...
        return final_output, counts, route_prob.sum(0), dropped, route_prob_max


class InfSwitchTransformerLayer(Module):
    """
    ...
    def __init__(self, *,
                 ...
                 feed_forward: InfSwitchFeedForward,
                 dropout_prob: float):
        """
    ...
    def forward(self, *,
        ...
        ff, counts, route_prob, dropped, route_prob_max = self.feed_forward(z, dropped)
        # Add the feed-forward results back
        x = x + self.dropout(ff)

        return x, counts, route_prob, dropped, route_prob_max


class InfSwitchTransformer(Module):
    """
    ## Switch Transformer
    """

    def __init__(self, layer: InfSwitchTransformerLayer, n_layers: int):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor):
        ...
        counts, route_prob, route_prob_max = [], [], []
        dropped = []
        steps = 0
        while len(dropped) < x.shape[0]*x.shape[1] and steps < 10:
            x, f, p, d, p_max = self.layer(x=x, mask=mask, dropped=dropped)
            counts.append(f)
            route_prob.append(p)
            dropped+=d
            dropped = list(set(dropped))
        ...
        x = self.norm(x)
        #
        return x, torch.stack(counts), torch.stack(route_prob), dropped, torch.stack(route_prob_max)
